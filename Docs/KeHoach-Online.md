...
Giao thức lên **7** (25/09): đồ boss nằm trong rương giữa đấu trường. Rương là một vật mạng mới (`SpawnKind.Chest`, `NetEntityKind.Chest`) mà máy chủ tạo cho từng người có công và chỉ gửi cho người đó, giống đồ rơi riêng. Máy chủ mở rương khi người đó tới gần (hoặc sau 150 giây) và tung đồ ra như đồ rơi thường (`EntityGone` kiểu `Collected` báo máy người chơi bật nắp). Tự Động chạy trên máy người chơi và đi qua cùng đường với chuột và phím, nên máy chủ không cần gì mới.

Giao thức lên **8** (25/09): lớp nhân vật, chủng tộc và ngoại hình (`Docs/NhanVat.md`). Nhân vật mang một `HeroLook` (lớp, chủng tộc, vũ khí, cấp rèn, các lựa chọn ngoại hình), lưu trong phần `player`. Người chơi gửi lựa chọn bằng `ActKind.ChooseLook`: máy chủ chỉ nhận lớp và chủng tộc một lần, ngoại hình và vũ khí lúc nào cũng được (`CharacterChoice`). `HeroInfoMsg` mang thêm ngoại hình, nên mọi màn hình tự vẽ nhân vật người khác (`HeroArt`) và gắn đúng bộ kỹ năng cho bản sao (để hiện đúng chiêu người đó tung). Lò Rèn là `ActKind.Forge`: máy chủ kiểm tra đứng cạnh Thợ Rèn và đủ vàng, vật liệu. Màn tạo nhân vật online chỉ mở khi phiếu nhân vật của máy chủ đã tới (sau `Ready`). Netsmoke thêm một bước: SmokeA chọn Du Hiệp Tiên Tộc, SmokeB phải thấy A đúng như thế, vòng hai A vào lại vẫn là Du Hiệp.
...
Giao thức lên **12** (26/09): Hắc Phong (T62, `Docs/ThaoNguyen.md` mục Hắc Phong). Bản đồ thêm Bù Nhìn Sống, cung thủ và đao thủ Hắc Phong, Bò Rừng Sắt, Thủ Lĩnh Hắc Phong với 4 cung thủ chờ trong `Brood`, và một đá truyền tống. Vẫn không có tin nhắn mới.
- **Bọn cướp là người**: máy chủ chạy chúng với bộ hoạt ảnh riêng của prefab (clip của quái xếp theo thứ tự cố định). Mỗi màn hình tự vẽ chúng theo một `HeroLook` (`HeroLookEnemy`, giống người chơi khác) và giữ nguyên thứ tự clip, nên số clip máy chủ gửi khớp. Máy chủ không vẽ gì.
- **Ba khóa đạn mới** cho `NetCues.Projectile`: `"arrow"`, `"windblade"`, `"tornado"`. Bản sao của lưỡi gió và lốc xoáy xuyên người như bản ở máy chủ, không vỡ ở người đầu tiên. Tất cả trôi theo cùng một cơn gió trên mọi máy.
- **Bù nhìn nhảy** bằng phần `lift` sẵn có (như chim ưng).
- **Bão Cát** không cần gửi gì thêm: mỗi máy nổi cát khi boss đang đánh và đã cuồng nộ, hai cờ mà máy chủ đã gửi sẵn. Gió riêng của đồi (12 giây) tính theo giờ trong ngày như gió thảo nguyên.
